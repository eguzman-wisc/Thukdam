...
# MMN and ABR files created and saved as .bdf files
logger.debug('Using pyedflib writer to begin writing MMN data .bdf file.\n')
m = pyedflib.EdfWriter(mmn_outfile, len(cropped_abr.info['ch_names']), file_type=pyedflib.FILETYPE_BDF)
...
for x in xrange(0,len(cropped_mmn.info['ch_names'])):
    mmn_chan_info = {'label':cropped_mmn.info['ch_names'][x], 'dimension': 'mV', 'sample_rate': freq, 'physical_max': 1.0, 'physical_min': -1.0, 'digital_max': 8388607, 'digital_min': -8388608, 'prefilter': 'LP:3334Hz', 'transducer': 'trans1'}
...
for x in xrange(0,len(mmn_chan_data)):
    ...
    mmn_data[x] = numpy.array(mmn_data[x])
    
for x in xrange(0,int(len(cropped_mmn)/freq)):
...
for x in xrange(0,len(cropped_abr.info['ch_names'])):
    abr_chan_info = {'label':cropped_abr.info['ch_names'][x], 'dimension': 'mV', 'sample_rate': freq, 'physical_max': 1.0, 'physical_min': -1.0, 'digital_max': 8388607, 'digital_min': -8388608, 'prefilter': 'LP:3334Hz', 'transducer': 'trans1'}
...
for x in xrange(0,len(abr_chan_data)):
    ...
    abr_data[x] = numpy.array(abr_data[x])
    
for x in xrange(0,int(len(cropped_abr)/freq)):
